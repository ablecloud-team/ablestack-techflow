...
assert data["decision"] == "GO_COMPLETED"
assert data["production_changed"] is True
...
assert data["production_rollout"]["final"]["flarum_core"] == "1.8.18"
assert data["production_rollout"]["final"]["discussions"] == 120
assert data["production_rollout"]["final"]["posts"] == 320
assert data["production_rollout"]["final"]["upload_files"] == 115
assert data["production_rollout"]["techflow_integration"]["protected_github_chat_changed"] is False
assert len(PdfReader(str(REPORT)).pages) >= 8
