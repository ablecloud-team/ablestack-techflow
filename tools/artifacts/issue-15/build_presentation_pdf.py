#!/usr/bin/env python3
"""Assemble rendered Issue #15 slide PNGs into a presentation PDF."""
...
SLIDES_DIR = ROOT / "tmp" / "artifacts" / "issue-15" / "rendered"
OUTPUT = ROOT / "output" / "pdf" / "techflow-secret-management-presentation.pdf"
...
def build() -> None:
    ...
    pdf.setTitle("TechFlow Secret 수명주기 검증")
    pdf.setAuthor("ABLESTACK TechFlow")
    for image_path in images:
        with Image.open(image_path) as image:
            image_width, image_height = image.size
        if (image_width, image_height) != (width, height):
            raise ValueError(f"Inconsistent slide size: {image_path} {(image_width, image_height)}")
