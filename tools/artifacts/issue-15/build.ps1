...
$workspace = Join-Path $repoRoot "tmp\artifacts\issue-15"
$templateInspect = Join-Path $workspace "template-inspect"
$rendered = Join-Path $workspace "rendered"
$sourceDeck = Join-Path $repoRoot "output\presentation\https-webhook-ingress.pptx"
$presentationOutput = Join-Path $repoRoot "output\presentation\techflow-secret-management.pptx"
...
$audit = @"
Source: output/presentation/https-webhook-ingress.pptx
Slides inspected: 10/10
Reusable patterns: title, outcome cards, responsibility cards, policy table, six-step runbook, trust flow, evidence cards, asset cards, roadmap, closing
...
Write-Host "Issue #15 artifacts generated."
