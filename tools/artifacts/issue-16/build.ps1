...
$workspace = Join-Path $repoRoot "tmp\artifacts\issue-16"
$templateInspect = Join-Path $workspace "template-inspect"
$rendered = Join-Path $workspace "rendered"
$sourceDeck = Join-Path $repoRoot "output\presentation\techflow-secret-management.pptx"
$presentationOutput = Join-Path $repoRoot "output\presentation\techflow-backup-recovery.pptx"
...
$audit = @"
Source: output/presentation/techflow-secret-management.pptx
Slides inspected: 10/10
Reusable patterns: title, outcome cards, responsibility cards, policy table, six-step runbook, isolation flow, evidence cards, asset cards, roadmap, closing
Typography and layout: preserve source font family, sizes, spacing, master/layout hierarchy, page markers and color system
Inherited placeholders: no unresolved structural placeholders observed; all mapped narrative text and table cells are rewritten in place
...
& $nodeExe $workspaceInspectSource $sourceDeck (Join-Path $templateInspect "template-inspect-full.ndjson")
...
try {
    & $nodeExe (Join-Path $skillRoot "template_following_scripts\prepare_template_starter_deck.mjs") `
        ...
        --out $starterDeck `
        --inspect (Join-Path $templateInspect "template-inspect-full.ndjson") `
...
Write-Host "Issue #16 artifacts generated."
