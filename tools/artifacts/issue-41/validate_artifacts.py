#!/usr/bin/env python3
"""Validate Issue #41 links, checksums, page counts, deck, and secret hygiene."""
...
MANIFEST = ROOT / "output/issue-41-artifact-manifest.json"
...
assert manifest["issue"] == 41
assert manifest["status"] == "implemented-and-validated"
...
assert len(PdfReader(str(ROOT / "output/pdf/techflow-ai-gateway-foundation-report.pdf")).pages) == 10
assert len(PdfReader(str(ROOT / "output/pdf/techflow-ai-gateway-foundation-presentation.pdf")).pages) == 10
with zipfile.ZipFile(ROOT / "output/presentation/techflow-ai-gateway-foundation.pptx") as archive:
    ...
    for expected in ("Issue #41", "11 API", "15 TABLES", "CANARY PASS"):
