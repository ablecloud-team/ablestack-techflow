...
SOURCE = ROOT / "docs/reports/issue-21-community-assist-validation.md"
OUTPUT = ROOT / "output/pdf/techflow-community-assist-report.pdf"
...
def inline(text: str) -> str:
    value = escape(text)
    value = re.sub(r"`([^`]+)`", r"<font name='Malgun'><b>\1</b></font>", value)
...
def page(canvas, doc) -> None:
    ...
    canvas.drawString(18 * mm, 10 * mm, "ABLESTACK TechFlow · Issue #21")
...
def styles():
    ...
    return {
        ...
        "body": ParagraphStyle("BodyKR", parent=base["BodyText"], fontName=REGULAR, fontSize=9.4, leading=15, textColor=colors.HexColor("#1F2937"), spaceAfter=2.2 * mm),
        "bullet": ParagraphStyle("BulletKR", parent=base["BodyText"], fontName=REGULAR, fontSize=9.2, leading=14, leftIndent=5 * mm, firstLineIndent=-3 * mm, spaceAfter=1.3 * mm),
        ...
        "code": ParagraphStyle("CodeKR", parent=base["Code"], fontName=REGULAR, fontSize=8.3, leading=12, backColor=colors.HexColor("#F3F4F6"), borderPadding=5, spaceAfter=2 * mm),
    }


def table_from(lines: list[str], style) -> Table:
...
def main() -> None:
    ...
    doc = BaseDocTemplate(str(OUTPUT), pagesize=A4, leftMargin=18 * mm, rightMargin=18 * mm, topMargin=18 * mm, bottomMargin=16 * mm, title="TechFlow Issue #21 Community Assist 검증 보고서")
