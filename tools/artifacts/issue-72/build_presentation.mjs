...

{
  ...
  text(s,"cover-subtitle","50 MiB 경계 · 압축 안전검사 · 운영 E2E",42,430,560,70,28,false,C.gray);
  box(s,"cover-hero",660,42,578,588,C.pale,C.rule);
  text(s,"cover-stat","50\nMiB",708,160,480,240,90,true,C.blue,"center");
  text(s,"cover-detail","파일당 운영 상한\n2026.08.15",708,430,480,100,26,false,C.gray,"center");
  notes(s);
}
{
  const s=deck.slides.add(); title(s,"하나의 50 MiB 경계가 전 계층을 관통합니다",2);
  const labels=["Nginx\n120 MiB","PHP-FPM\n64 MiB","FoF Upload\n50 MiB","Poller\n50 MiB","Gateway\n50 MiB","압축 해제\n100 MiB"];
  ...
  text(s,"flow-foot","다운로드 120초 · 일시 오류 2회 재시도 · 100개 항목 · 압축비 20배 · 24시간 보관",42,500,1196,62,24,false,C.gray,"center"); notes(s);
}
{
  const s=deck.slides.add(); title(s,"병목은 10 MiB에서 50 MiB로 이동했습니다",3);
  const values=[["계층","적용 전","적용 후"],["FoF Upload","10 MiB","50 MiB"],["PHP-FPM","120 MiB · 30/60초 · 128 MiB","64 MiB · 300/300초 · 256 MiB"],["Poller","10 MiB · 30초 · 재시도 없음","50 MiB · 120초 · 2회"],["Gateway","원본 10 / 해제 20 MiB","원본 50 / 해제 100 MiB"],["유지관리","수동","15분 · 24시간 · 70%/85%"]];
...
{
  const s=deck.slides.add(); title(s,"운영 검증은 코드, 경계, 실제 대화를 모두 통과했습니다",4);
  const cards=[{v:"259/259",d:"전체 런타임 회귀"},{v:"50 MiB",d:"허용 · +1 byte 거부"},{v:"4종",d:"로그 · ZIP · GZIP · TAR.GZ"}];
  ...
  text(s,"metric-caption","Discussion #172: Artifact 4개 수집 · AI 답변 자동 게시 · 시험 데이터 영구 삭제",42,160,1196,72,25,false,C.gray,"center"); notes(s);
}
{
  ...
  box(s,"security-side",850,176,388,420,C.pale,C.blue); text(s,"security-side-title","운영 안전망",884,220,320,45,28,true,C.blue);
  text(s,"security-side-body","24시간 만료 삭제\n15분 주기 점검\n70% 경고\n85% 위험\n\n최초 실행에서\n만료 28개 정리",884,292,320,250,25,false,C.ink); notes(s);
}
{
  const s=deck.slides.add(); s.background.fill=C.white;
  text(s,"close-kicker","ISSUE #72 · COMPLETE",42,42,420,42,20,true,C.gray);
  text(s,"close-title","운영 상태는\nGO입니다",42,188,850,190,82,true);
  text(s,"close-detail","Gateway healthy · Poller running · Maintainer running\nGitHub→Chat 보호 서비스 frozen / guard passed",42,485,760,85,28,false,C.gray);
