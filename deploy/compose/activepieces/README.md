...
- GitHub 조직 Push·PR Merge HMAC 검증과 Synology Chat 내부 Adapter
- Activepieces Worker가 접근할 제어 API·Chat Adapter만 허용하는 격리 네트워크
...
실제 설치와 운영 절차는 [Activepieces Compose 배포 Runbook](../../../docs/runbooks/activepieces-compose-deployment.md), [HTTPS·Webhook Ingress 운영 Runbook](../../../docs/runbooks/https-webhook-ingress.md), [GitHub·Synology Chat 자동화 Runbook](../../../docs/runbooks/github-chat-webhook.md), [상태 백업·복구 Runbook](../../../docs/runbooks/state-backup-recovery.md)과 [이미지 버전 업그레이드·롤백 Runbook](../../../docs/runbooks/image-version-upgrade-rollback.md)을 따른다.
...
sudo --preserve-env=TECHFLOW_GITHUB_WEBHOOK_SECRET \
  python3 scripts/verify-github-chat.py --flow-id BjLSSjbSp3tyNLjnxWUS8
```
...
일상 배포에서는 `docker compose up --build`나 mutable Tag를 직접 사용하지 않는다. `image-lock.json`을 검토한 뒤 `deploy-locked.sh`로 배포하고, 장애 시 `/var/lib/ablestack-techflow/releases/runtime-lock.previous.json`을 `rollback-release.sh`에 전달한다. Event Gateway의 현재 M1 릴리스는 테스트 서버에 사전 빌드된 Image ID를 사용하며 고객 배포 전 승인 Registry Digest로 전환한다.
