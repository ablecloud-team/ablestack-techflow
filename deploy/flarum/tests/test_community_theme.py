...
class CommunityThemeContractTests(unittest.TestCase):
    ...
    def test_tag_date_format_contract(self) -> None:
        ...
        self.assertIn("!document.querySelector('.Composer.visible:not(.minimized)')", self.forum_js)
    ...
    def test_compact_hero_and_navigation_width_contract(self) -> None:
        ...
        self.assertIn(".App:has(.Composer.visible:not(.minimized)) .DiscussionPage-list", self.less)
        self.assertIn(".App.hasPane.panePinned:has(.Composer.visible:not(.minimized)) .App-composer", self.less)
        ...
        self.assertIn("bottom: 20px", self.less)
        self.assertIn("padding-bottom: 32px", self.less)
