...
게시물 답장은 40px 아이콘 버튼으로 항상 표시한다. DOM의 `답장` 문구는 접근 가능한 이름으로 유지하며, 비로그인처럼 현재 답장할 수 없는 상태는 회색 배경·45% 투명도·금지 커서·포인터 실행 차단으로 구분한다. 활성 상태는 블루 계열 배경과 3px 포커스 링을 사용한다.
...
최종 실행 ID는 `issue73-20260819-hero-avatar-actions-final`이다.
...
| 게시물 작업 버튼 | 답장 아이콘 상시 표시, 비로그인 비활성 상태, 40px/모바일 44px | PASS |
