...
게시물 작업은 답장·좋아요·해결 선택·더 보기를 모두 아이콘과 한글 텍스트 조합으로 표시한다. 카드 경계에 걸치던 위치를 카드 아래 8px의 독립 작업 영역으로 옮기고 오른쪽 선을 카드와 맞췄다. 작업 그룹은 Flex로 배치하고 데스크톱 8px, 모바일 6px 간격을 적용해 기존 5px 겹침을 0px로 해소했다. 모바일 390px에서도 네 버튼이 한 줄에 유지되며 가로 넘침은 0px, 다음 게시물과의 최소 간격은 5.28px다. 버튼 뒤를 지나던 Flarum 기본 게시물 구분선은 카드 보더와 역할이 중복되어 제거했다. 비로그인처럼 현재 답장할 수 없는 상태는 회색 배경·45% 투명도·금지 커서·포인터 실행 차단으로 구분한다.
...
최종 실행 ID는 `issue73-20260819-dialog-action-consistency-final-rerun`이다.

| 단계 | Theme | HTTP | 사용자/토의/게시물 | 콘텐츠 해시 | 첨부 해시 |
|---|---|---:|---:|---|---|
| 기준선 | Disabled | 200 | 39 / 117 / 305 | `55ed12c8...ad9d` | `19cdf526...97c` |
...
| 게시물 작업 버튼 | 아이콘+한글 텍스트 통일, 카드 아래 8px 외부 배치, 데스크톱 8px·모바일 6px 간격, 겹침·넘침 0px | PASS |
