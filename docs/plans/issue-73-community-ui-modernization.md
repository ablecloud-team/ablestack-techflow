...
| 로그인 헤더 | 검색·언어·신고·알림·프로필의 높이가 다르고 미매핑 아이콘이 점으로 표시 | 다섯 항목을 44px·동일 Y축으로 정렬하고 신고·알림을 내장 SVG로 표시 |
...
- 로그인 상태의 검색·언어·신고·알림·프로필은 44px 높이와 같은 세로 중심선을 사용하며, 신고·알림은 웹폰트와 무관한 단색 SVG 아이콘을 사용한다.
