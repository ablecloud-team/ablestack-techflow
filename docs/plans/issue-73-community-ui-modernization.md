...
| 로그인 헤더 | 검색·언어·신고·알림·프로필의 높이가 다르고 미매핑 아이콘이 점으로 표시 | 다섯 항목을 44px·동일 Y축으로 정렬하고 사용자 메뉴까지 공통 SVG 아이콘 적용, 드롭다운 빈 아이콘·Hover 이동 제거 |
...
- 로그인 상태의 검색·언어·신고·알림·프로필은 44px 높이와 같은 세로 중심선을 사용하며, 신고·알림·프로필·설정·관리·로그아웃은 웹폰트와 무관한 공통 단색 SVG 아이콘을 사용한다. 언어·사용자 드롭다운은 비선택 항목의 빈 아이콘을 숨기고 기본·Hover 패딩을 `10px 12px`로 고정해 메뉴 경계를 벗어나지 않는다.
...
- 언어 드롭다운의 검은 사각형은 0건이고 기본·Hover 항목의 좌우 넘침은 0px이다.
