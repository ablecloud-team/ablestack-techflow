...
| 카테고리 | 왼쪽 메뉴가 190px로 좁고 태그 페이지 버튼이 컨테이너를 넘음 | 280px 메뉴, 태그 도구 모음 버튼 넘침 0px |
| 태그 목록 | 태그가 색상 표처럼 서로 붙어 있고 최근 토론·보조 태그 정보가 구분되지 않음 | 주요 태그 독립 카드, 최근 토론 패널, 설명을 포함한 별도 보조 태그 구역 |
| 아이콘 | Font Awesome 웹폰트가 로드되지 않으면 전용 코드가 점·더하기·깨진 사각형으로 표시 | 의미 Class를 유지한 공통 내장 SVG Mask로 웹폰트·운영체제 기호 의존 제거. 배지·타임라인·댓글 수·설정 알림·작성기·검색 초기화 포함 |
| 로그인 헤더 | 검색·언어·신고·알림·프로필의 높이가 다르고 미매핑 아이콘이 점으로 표시되며 드롭다운 아이콘이 카드 바깥으로 밀림 | 다섯 항목을 44px·동일 Y축으로 정렬하고 사용자 메뉴까지 공통 SVG 아이콘 적용, 180px 드롭다운과 19px 고정 아이콘 열로 빈 아이콘·음수 여백·Hover 이동 제거 |
| 좌측 메뉴 | 카테고리·프로필 메뉴가 마우스와 키보드 입력에 거의 반응하지 않아 메뉴로 인지하기 어려움 | 16px 글자·18px 아이콘·행 중앙 정렬과 배경·독립 강조선·2px 이동을 hover·focus·active 상태로 구분 |
| Header·Footer 배치 | Header는 중앙 1100px, Footer는 화면 전체 폭을 기준으로 배치되어 좌우 기준선이 다름 | Header·본문·Footer를 동일한 1100px 컨테이너와 15px 내부 여백에 정렬 |
| 목록 | 제목·태그·댓글 수가 여러 줄에 흩어지고 아바타가 제목보다 아래에 있음 | 데스크톱 62px 단일 행, 아바타·제목 중심 일치, 모바일 2줄 제목 |
| 목록 작업 | 더보기 버튼이 Hover 상태에서만 보이고 열린 메뉴가 다음 행 뒤로 들어감 | 테두리·배경 없는 이클립스 아이콘 상시 표시·세로 중앙 정렬, 열린 행과 메뉴의 명확한 최상위 계층 |
| 상세 목록 패널 | 토론 목록 패널이 Header와 15px 겹치고 행·스크롤바가 기본 스타일로 표시 | Header 아래 68px에서 시작, 400px 카드 목록, 6px 얇은 스크롤바, 현재 토론 강조 |
...
| 프로필 | 기본 회색 Hero, 좁은 메뉴, `Likes`·`My media`·`Security` 영문 노출 | 블루 Hero·280px 메뉴·카드형 빈 상태와 프로필 메뉴 한글화 |
...
- 로그인 상태의 검색·언어·신고·알림·프로필은 44px 높이와 같은 세로 중심선을 사용하며, 신고·알림·프로필·설정·관리·로그아웃은 웹폰트와 무관한 공통 단색 SVG 아이콘을 사용한다. 언어·사용자 드롭다운은 폭 180px, 내부 8px, 행 패딩 `10px 12px`, 19px 고정 아이콘 열과 9px 간격을 사용한다. Flarum 기본 절대 위치와 음수 왼쪽 여백은 제거하고 비선택 항목은 아이콘 자리 폭을 유지한 채 숨겨 모든 글자의 시작선과 세로 중심을 맞춘다.
- 환영 영역은 포럼의 목적을 한 문장으로 설명하고 목록과 배경을 분리한다.
- 환영 타이틀은 34px 로고 높이보다 작은 28px로 표시하고 Hero 전체 높이는 약 128px로 제한한다.
- 좌측 탐색은 280px 폭에서 모든 토의, 태그, 제품 영역 순으로 유지한다. 메뉴 글자는 16px, 아이콘은 18px로 키우고 46px 행의 세로 중앙에 배치한다. 마우스 hover와 키보드 focus는 옅은 블루 배경·독립된 좌측 3px 선·오른쪽 2px 이동·아이콘 확대를 사용하고, 선택 상태는 Primary 테두리와 강조선으로 구분한다.
- 태그 페이지의 `토의 시작` 버튼은 내부 패딩을 포함한 도구 모음 경계 안에 유지한다. 태그 도구 모음의 아이콘과 글자는 10px 간격으로 같은 중심선에 배치하고, 선택 강조선은 콘텐츠와 겹치지 않는 버튼 하단에 표시한다.
...
- 목록에서 토론을 열 때 마지막 게시물 번호가 포함된 링크를 토론 기본 주소로 정규화해 항상 첫 게시물부터 보여준다. 알림과 상세 내부의 특정 답변 링크는 변경하지 않는다.
- 작성자 아바타의 세로 중심을 제목과 일치시켜 행 내부의 시각 기준선을 통일한다.
- 모바일은 제목 2줄과 마지막 응답 정보를 유지해 좁은 화면에서 의미가 사라지지 않도록 한다.
- 각 행의 더보기 버튼은 마우스를 올리지 않아도 보이게 하고 행 높이의 세로 중앙에 배치한다. 기본 상태는 테두리·배경·그림자 없이 세로 이클립스 아이콘만 표시하고, hover·focus·열림 상태에서만 옅은 원형 배경을 사용한다. 열린 행, 버튼, 메뉴의 `z-index`를 단계적으로 높여 뒤쪽 행이 메뉴를 가리지 않게 한다.
- 상세 화면의 토론 목록 패널은 68px Header 아래에서 시작하고 높이는 `100vh - 68px`로 제한한다. 패널은 400px 폭의 카드 목록, 8px 행 간격, 6px 스크롤바를 사용하며 현재 토론을 Primary 선과 옅은 블루 배경으로 구분한다.
...
- 데스크톱 프로필 메뉴는 홈 카테고리와 같은 280px 폭, 카드 경계와 블루 활성 상태를 사용한다.
- 프로필·설정·보안 메뉴에도 홈 카테고리와 같은 hover·focus·active 인터랙션을 적용한다.
...
- 데스크톱 Footer는 Header·본문과 같은 1100px 컨테이너와 좌우 15px 여백을 사용하고 모바일에서는 세 링크 그룹을 중앙에 배치한다.

### 3.9 설정과 보안

- 설정과 보안 본문은 프로필의 280px 탐색 옆에 같은 폭과 간격으로 배치하고, 각 설정 묶음은 16px Radius의 흰색 카드로 구분한다.
...
아이콘은 Flarum의 `fa-*` 의미 Class를 유지하되 CSS에 포함한 단색 SVG Mask로 표시한다. 메뉴·프로필·설정·보안·목록 작업·태그·Footer뿐 아니라 연구소 배지, 토론 타임라인, 댓글 수의 Core 가상 요소, 설정 알림, 작성기 전송·이미지·이모지, 브라우저 기본 검색 초기화까지 같은 24px ViewBox와 2px 선 굵기 체계를 사용하므로 외부·로컬 웹폰트, 운영체제 기호 글꼴과 Cache 재생성 순서에 영향을 받지 않는다.

검색 입력은 공통 포커스 외곽선과 입력 전용 링을 중복 적용하지 않고, 모서리 왜곡이 없는 2px 브랜드 링 하나만 사용한다. 관리자·기술지원 멤버 배지는 렌치·번개 SVG를 사용한다.
...
- 절대 날짜는 `00년 0월 0일`, 상대 날짜는 기존 표현으로 표시되며 화면의 점·더하기·깨진 사각형 대체 아이콘은 0건이다. 토론 정렬 메뉴는 선택 항목의 SVG 체크만 보이고 비선택 항목의 빈 아이콘 자리는 투명하게 유지한다.
- 로그인 헤더의 다섯 항목은 높이 44px·Y축 편차 0px이고 신고·알림 아이콘이 항상 표시된다.
- 언어·사용자 드롭다운의 검은 사각형과 카드 밖 아이콘은 0건이고 기본·Hover 항목의 좌우 넘침은 0px이다. 두 메뉴의 글자 시작선은 각각 동일하며 아이콘-글자 중심 편차는 1px 이하이다.
- 카테고리·프로필·설정·보안 좌측 메뉴는 hover·focus·active 상태가 배경·강조선·이동·아이콘 변화로 구분된다.
- 토론 목록의 더보기 버튼은 테두리·배경·그림자 없이 이클립스 아이콘만 항상 보이고 세로 중심 오차가 0px이며 열린 메뉴는 이후 목록 행보다 위에 표시된다.
- 토론 목록 진입 링크에는 게시물 번호가 없고 상세 시작 `scrollY`는 0이며 첫 게시물이 화면에 보인다.
- 상세 토론 목록 패널의 Header 겹침은 0px, 스크롤바는 6px, 카드 간격은 8px이며 현재 토론이 명확히 구분된다.
- 검색 입력 포커스 외곽선은 `none`, 단일 링은 2px이고 관리자·기술지원·연구소 배지와 검색 초기화 버튼이 깨진 사각형 없이 표시된다.
- 토론 타임라인 처음·끝 이동, 댓글 수, 설정의 이메일·알림 시계, 작성기의 전송·이미지·이모지 아이콘은 모두 유효한 내장 SVG Mask를 사용한다.
...
- Footer 링크가 원형 SVG 아이콘으로 구분되고 마지막 콘텐츠를 가리지 않으며 Header·본문과 좌우 기준선이 일치한다.
...
- 프로필 Hero·280px 메뉴·빈 상태 카드가 전체 테마와 일치하며 프로필 메뉴 직접 노출 영문이 0건이다.
