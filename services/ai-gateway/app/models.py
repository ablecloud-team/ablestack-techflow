...
class ComprehensiveQueryRequest(StrictModel):
    ...
    compatibility_set_id: UUID | None = Field(default=None, alias="compatibilitySetId")
    source_profile_ids: list[SafeId] | None = Field(default=None, min_length=1, max_length=9, alias="sourceProfileIds")
    artifact_ids: list[UUID] = Field(default_factory=list, max_length=20, alias="artifactIds")
    required_artifact_ids: list[UUID] | None = Field(default=None, max_length=20, alias="requiredArtifactIds")
...
class CommunityCaseCreateRequest(StrictModel):
    ...
    artifact_warnings: list[Annotated[str, StringConstraints(min_length=3, max_length=300)]] = Field(
        default_factory=list, max_length=20, alias="artifactWarnings"
    )
